...
--Select matches where Wales was ‘Team 2’
SELECT s.id, t.name,
CASE WHEN team1_goal > team2_goal THEN 'Wales lose!'
WHEN team1_goal < team2_goal THEN 'Wales win!'
ELSE 'Draw' END AS Outcome
FROM scores s
JOIN teams t
ON s.team1_id = t.id
...
--Identify when Belgium won a match in Euro 2016
SELECT m.date, s.name, v.name, 
CASE WHEN scores.team1_id = 6 AND team1_goal > team2_goal THEN 'Belgium win'
WHEN scores.team2_id = 6 AND team2_goal > team1_goal THEN 'Belgium win'
END AS outcome
FROM scores
JOIN match m
ON scores.id = m.id
